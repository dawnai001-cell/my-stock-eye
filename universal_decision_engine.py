...
if len(buy_signals) > 0 and len(sell_signals) > 0:
    ...
    for b, s in zip(buy_signals[:min_len], sell_signals[:min_len]):
        if s > b:
            trade_returns.append((data['Close'].iloc[s] - data['Close'].iloc[b]) / data['Close'].iloc[b])

sharpe = (returns.mean() / returns.std() * np.sqrt(252)) if returns.std() != 0 else 0
total_return = cumulative_return.iloc[-1] * 100 if len(cumulative_return) > 0 else 0
max_drawdown = (cumulative_return - cumulative_return.cummax()).min() * 100 if len(cumulative_return) > 0 else 0
...
col2.metric("📊 夏普比率", f"{sharpe:.3f}")
col3.metric("📉 最大回撤", f"{max_drawdown:.2f}%")
col4.metric("🎯 勝率", f"{win_rate:.1f}%")
col5.metric("🔄 交易次數", f"{len(buy_signals)}")
st.markdown("---")
...
fig = make_subplots(
    ...
    cols=1, 
    shared_xaxes=True,
    subplot_titles=('📈 股價 K 線與買賣點', '📊 MACD', '📉 J 值'),
...
fig.add_trace(
    go.Candlestick(
        ...
        close=data['Close'],
        name='K 線',
        increasing_line_color='#ff4d4d',  # 紅 K（台股習慣）
        decreasing_line_color='#00cc66'   # 綠 K（台股習慣）
    ),
    row=1, col=1
)
...
if buy_signals:
    fig.add_trace(
        go.Scatter(
            ...
            y=data['Low'].iloc[buy_signals] * 0.97,
            mode='markers',
            name='買進',
            marker=dict(
                color='#00cc66',
...
if sell_signals:
    fig.add_trace(
        go.Scatter(
            ...
            y=data['High'].iloc[sell_signals] * 1.03,
            mode='markers',
            name='賣出',
            marker=dict(
                color='#ff4d4d',
...
fig.add_trace(
    go.Bar(
        ...
        y=hist,
        name='柱狀體',
        marker_color=['#ff4d4d' if h < 0 else '#00cc66' for h in hist],
        opacity=0.6
    ),
    row=2, col=1
)
...
# 門檻線
fig.add_hline(y=j_threshold, line_dash="dash", line_color="#ff4d4d", opacity=0.7, row=3, col=1)
...
with st.expander("📋 交易記錄"):
    if len(buy_signals) > 0 and len(sell_signals) > 0:
        min_len = min(len(buy_signals), len(sell_signals))
        buy_dates = data.index[buy_signals[:min_len]]
        sell_dates = data.index[sell_signals[:min_len]]
        buy_prices = data['Close'].iloc[buy_signals[:min_len]].values
        sell_prices = data['Close'].iloc[sell_signals[:min_len]].values
        trade_returns_pct = (sell_prices - buy_prices) / buy_prices * 100
        ...
        trade_df = pd.DataFrame({
            ...
            '賣出日期': sell_dates,
            '賣出價格': sell_prices,
            '報酬%': trade_returns_pct
        })
        st.dataframe(trade_df, use_container_width=True)
    else:
        st.info("📭 無交易記錄")def load_data(ticker, start, end):
    raw = yf.download(ticker, start=start, end=end, progress=False)
    data = pd.DataFrame(index=raw.index)
    data['Open'] = raw['Open'].values
    data['High'] = raw['High'].values
