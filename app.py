...
def load_stock_data(ticker):
    # 由於最大可能觀測 200 根 K 棒，故將 yfinance 的抓取範圍擴大至 1 年（1y），確保數據充足
    try:
...
# 💡 動態調整 X 軸日期的密度，避免放大到 200 根時文字重疊擁擠
step = max(1, len(df) // 8)
...
ax1.vlines(df['x_index'], df['Low'], df['High'], color='#aaaaaa', linewidth=1.0)

# 💡 K 棒寬度隨根數動態調整（天數越多棒子越細，避免畫面擠成一團）
k_width = 4.5 if check_days > 100 else 6
...
with tab1:
    ...
    ax_vol.bar(df['x_index'], df['Volume'], color=colors, width=0.5 if check_days > 100 else 0.6, alpha=0.9)
...
with tab3:
    ...
    ax_obv.plot(df['x_index'], df['OBV'], color='#00ffff', linewidth=1.8, label='OBV Flow')
    ax_obv.plot(df['x_index'], df['OBV_MA5'], color='#ffff00', linestyle=':', linewidth=1.2, label='OBV MA5')
