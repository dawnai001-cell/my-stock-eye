...
ticker_input = st.sidebar.text_input("請輸入台股代號（如 2317 或 2417）：", value="2317").strip()
check_days = st.sidebar.slider("請選擇觀測天數：", min_value=5, max_value=90, value=20, step=5)

st.sidebar.write("---")
st.sidebar.info("💡 備用大腦已升級：\n週末或證交所休息時，系統會自動無縫接軌 Yahoo Finance 真實歷史數據，確保切換代號時能看到正確的開高低收實體波動！")

# ==========================================
# 📊 數據獲取與計算（完美整合雙數據源）
# ==========================================
@st.cache_data(ttl=3600)  # 快取機制，避免頻繁抓取
def load_stock_data(ticker):
    # 優先嘗試從 Yahoo Finance 抓取最穩定的歷史真實數據（台股代號要加 .TW）
    yf_ticker = f"{ticker}.TW"
    try:
        # 為了計算指標，多抓一些天數
        df_yf = yf.download(yf_ticker, period="1mo" if check_days <= 15 else "6mo", progress=False)
        if not df_yf.empty and len(df_yf) > 0:
            ...
            df['Volume'] = df_yf['Volume'].astype(float) / 1000  # 轉為張數（yfinance預設為股數）
            return df.dropna(), "yf"
...
df_all, data_source = load_stock_data(ticker_input)

if df_all is None or len(df_all) == 0:
    st.error("❌ 無法取得該股票數據，請檢查代號是否輸入正確（例如：2317）。")
    st.stop()
...
df_all['Is_Up'] = df_all['Close'] >= df_all['Open'] # 真正的當日K線漲跌判定
...
date_labels = df.index.strftime('%m-%d').tolist() # 縮短日期格式成 月-日 看得更舒服
step = max(1, len(df) // 6)
...
# 畫實體棒（精準對齊真實 Open 與 Close，呈現長短胖瘦變化！）
